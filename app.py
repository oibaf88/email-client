...
def list_messages(folder: str) -> list[dict]:
    ...
    try:
        ...
        # ⚡ Bolt: Batch IMAP FETCH to prevent N+1 query problem.
        # Instead of fetching one message at a time, we request all selected UIDs in a single command.
        uid_str = ",".join(decode_bytes(uid) for uid in selected_uids)
        ...
        if status == "OK":
            ...
            for item in fetch_data or []:
                if isinstance(item, tuple):
                    ...
                    uid_match = re.search(
                        rb"UID\s+(?P<uid>\d+)", response_part, re.IGNORECASE
                    )
                    if uid_match:
                        parsed_uid = decode_bytes(uid_match.group("uid"))
                        parsed_flags = parse_flags(response_part)
                        batch_results[parsed_uid] = (payload, parsed_flags)

            # Preserve the original sort order (newest first)
            for raw_uid in selected_uids:
                uid = decode_bytes(raw_uid)
                if uid in batch_results:
                    raw_headers, flags = batch_results[uid]
                    messages.append(
